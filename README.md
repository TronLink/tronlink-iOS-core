...
Each address is mapped to a random UUID stored **locally only** in FMDB. Legacy `UserDefaults` data is migrated once and removed immediately; if that database write fails, the legacy copy is removed and mappings are regenerated by the app. Runtime mappings are never written to `UserDefaults`. The backend never sees the address, and the same user's two wallets appear as two independent UUIDs (no server-side linkage).
